...
import pandas as pd
import numpy as np
ROOT=Path('/Users/ray/Research/AUSSEF'); OUT=ROOT/'Experiment 4/audit/results'
...
inv=[]; schema=[]
for p in files:
 rel=str(p.relative_to(ROOT));inv.append(dict(path=rel,extension=p.suffix,bytes=p.stat().st_size,sha256=hashes.get(rel),role='generated_index_not_independent_source' if 'graphify-out' in rel else 'existing_repository_file'))
 if p.suffix=='.csv':
  try:
