...
import pandas as pd
ROOT=next(p for p in [Path(__file__).resolve().parent,*Path(__file__).resolve().parents] if (p/'NSW Data Panel.csv').exists()); O=ROOT/'Experiment 2/data/fiscal_panel_v2'
...
Run `python build_panel_v2.py`, then `python write_v2_reports.py`, with pandas, NumPy, matplotlib, xlrd and openpyxl installed. Scripts read the frozen original CSV and the previously cached official OLG workbooks in `Experiment 1/audit/operating_ratio_audit/sources/`; new report sources are cached in `Experiment 2/data/fiscal_panel_v2/sources/`. The project root is discovered from the repository layout. No downloads are needed for this rebuild. Source manifests retain failed retrievals as well as successes; a failed download is not a missing council observation.
...
inv=[]
for mf in [ROOT/'Experiment 1/audit/operating_ratio_audit/source_downloads.json',*O.glob('source_manifest*.json')]:
