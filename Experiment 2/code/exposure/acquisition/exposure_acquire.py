...
R=next(p for p in [Path(__file__).resolve().parent,*Path(__file__).resolve().parents] if (p/'NSW Data Panel.csv').exists()); O=R/'Experiment 2/data/disaster_exposure_v2';S=O/'sources';S.mkdir(parents=True,exist_ok=True)
