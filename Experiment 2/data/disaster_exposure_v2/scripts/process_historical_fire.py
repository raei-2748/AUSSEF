...
cs=pd.read_csv(R/'outputs/fiscal_panel_v2/council_selection.csv');cs=cs[cs.included].sort_values('council_key');keys=cs.council_key.tolist();look={norm(r.council_name):r.council_key for r in cs.itertuples()};look['nambuccavalley']=look['nambucca']
for yr in [2015,2023]:
 f=S/f'lga_{yr}.zip'
